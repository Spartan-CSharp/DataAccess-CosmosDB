...
namespace DataAccessConsoleUI
{
	internal static class Program
	{
		...
		{
			Console.WriteLine();
			Console.WriteLine("Welcome to Document Database CRUD!");
		...
		private static async Task<AddressModel> RetrieveAddressAsync()
		{
			...
			if ( addresses.Count > 0 )
			{
				...
				do
				{
					...
					{
						Console.WriteLine($"{i + 1}: {addresses[i].StreetAddress}, {addresses[i].City}, {addresses[i].State}  {addresses[i].ZipCode}");
					}

					selectionvalid = int.TryParse(Console.ReadLine(), out addressid);

					if ( !selectionvalid || addressid == 0 || addressid > addresses.Count )
					{
						Console.WriteLine("That is not a valid selection.");
					}
					else
					{
						output = addresses[addressid - 1];
					}
				} while ( !selectionvalid || addressid == 0 || addressid > addresses.Count );
			}
			else
			{
				Console.WriteLine();
		...
		private static async Task<EmployerModel> RetrieveEmployerAsync()
		{
			...
			if ( employers.Count > 0 )
			{
				...
				do
				{
					...
					{
						Console.WriteLine($"{i + 1}: {employers[i].CompanyName}");
					}

					selectionvalid = int.TryParse(Console.ReadLine(), out employerid);

					if ( !selectionvalid || employerid == 0 || employerid > employers.Count )
					{
						Console.WriteLine("That is not a valid selection.");
					}
					else
					{
						output = employers[employerid - 1];
					}
				} while ( !selectionvalid || employerid == 0 || employerid > employers.Count );
			}
			else
			{
				Console.WriteLine();
		...
		{
			Console.WriteLine();
			Console.WriteLine("Thank you for using Document Database CRUD!");
			Console.WriteLine();
		}
	}
}
